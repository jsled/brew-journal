...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        def wrapAndSave(x, cls):
            if cls._meta.get_field('notes').max_length and len(x.notes) + 12 >= cls._meta.get_field('notes').max_length:
                print 'skipping',cls,'id',x.id,'notes len',len(x.notes),'max_length',cls._meta.get_field('notes').max_length
                return
            x.notes = '~~~~~\n' + x.notes + '\n~~~~~'
            x.save()

        [wrapAndSave(recipe, orm.Recipe) for recipe in orm.Recipe.objects.filter(notes__isnull=False)]
        [wrapAndSave(brew, orm.Brew) for brew in orm.Brew.objects.filter(notes__isnull=False)]
        # these are naturally "oneline" or short, little/no expectation of rich formatting.
        # [wrapAndSave(step, orm.Step) for step in orm.Step.objects.filter(notes__isnull=False)]
        # [wrapAndSave(res, orm.BjcpCompetitionResults) for res in orm.BjcpCompetitionResults.objects.filter(notes__isnull=False)]
        # [wrapAndSave(sheet, orm.BjcpBeerScoresheet) for sheet in orm.BjcpBeerScoresheet.objects.filter(notes__isnull=False)]
